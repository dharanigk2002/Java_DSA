            ...
            if(land[r1][c1]==1) {
                for(r2=r1;r2<m && land[r2][c1]==1; r2++)
                    for(c2=c1;c2<n && land[r2][c2]==1; c2++)
                        land[r2][c2]=0;
                farms.add(new int[]{r1, c1, r2-1, c2-1});
            }
    }
    return farms.stream().toArray(int[][]::new);
}
void main() {
    int[][] grid={
            {1, 0, 0},
    ...
    int[][] farmLands=findFarmland(grid);
    for(int[] farm:farmLands)
        IO.println(Arrays.toString(farm));
}